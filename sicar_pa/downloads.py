...
def _headers_demonstrativo(token: str, cookie_cf: str) -> dict[str, str]:
    headers = {
        "accept": "*/*",
...
def baixar_demonstrativo_pdf(reg: dict[str, Any], cookie_cf: str, token: str) -> bytes | None:
    """Baixa o PDF do Demonstrativo (PRA) de um único imóvel.
    ...
    try:
        res = requests.get(url_download, headers=_headers_demonstrativo(token, cookie_cf), timeout=30)
...
def gerar_zip_demonstrativos_lote(
    ...
    """Baixa os Demonstrativos em PDF de vários imóveis e compacta em um zip único."""
    buffer = io.BytesIO()
    total = len(registros)
    ...
    with zipfile.ZipFile(buffer, "w", zipfile.ZIP_DEFLATED) as zip_mestre:
        for idx, reg in enumerate(registros, 1):
            ...
            bin_data = baixar_demonstrativo_pdf(reg, cookie_cf, token)
            if bin_data:
                zip_mestre.writestr(f"{codigo_car}/{nome_arquivo}", bin_data)
            time.sleep(0.3)
    buffer.seek(0)
    return buffer
